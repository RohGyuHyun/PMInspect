...
namespace NSLogSubsystem
{
	...
	public:
		...
		// Set/get log level threshold
		virtual EVENT_SEVERITY __stdcall GetLogLevelThreshold () const  = 0 ;
		virtual void __stdcall SetLogLevelThreshold ( EVENT_SEVERITY es ) = 0 ;

		// Stop causes ignoring all subsequent CreateRecords calls and flushing internal queue.
		// Returns when internal queue is empty.
		// Logger object can not be re-initialized after this call.
...
extern NSLogSubsystem::ILogSubsystem * g_pLogger ;

inline bool LogRecord ( const char *									szSourceName,			
						NSLogSubsystem::ILogSubsystem::EVENT_SEVERITY	Severity,
						const char *									szDescription,
						bool											bDontShowPopup = true
					  )
{
	bool bRes = false ;
	...
	if ( g_pLogger != NULL )
	{
		bRes = S_OK == g_pLogger -> CreateRecord ( szSourceName, Severity, szDescription, bDontShowPopup, NULL ) ;
	}
