...
#pragma warning( suppress : 4355 )   // warning C4355: 'this' : used in base member initializer list
	CEpp () : Tserial(), m_pLogger(NULL), p_commBuff(NULL), m_ooa(*this)
	{
...
#pragma warning( suppress : 4355 )	// warning C4355: 'this' : used in base member initializer list
	CEpp (CLogger *pLogger) : Tserial(), p_commBuff(NULL), m_ooa(*this)
	{
		m_pLogger = pLogger;       
		m_pFun = (ErrorFunctionPtr)0;
        m_pFunLocale = 0;
...
public:
	CWCriticalSection() : m_bIsLocalCs(false) {
		InitializeCriticalSection(&m_cs);
	}
	CWCriticalSection(CRITICAL_SECTION &cs) : m_bIsLocalCs (true) {
		m_cs = cs;
	}
	~CWCriticalSection() {
...
private:
	...
	static CWCriticalSection gs_cs;					// to faciliate table identification
