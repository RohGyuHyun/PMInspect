...
// To start distance based strobes at even separation based on triggers from mfc. 
// - iNum number of strobes,
// - iDistance100um, distance expressed in 100um units (127max) 
int atf_XyStrobeStart(int iNum, int iDistance100um);
// To start free running strobes. 
// Note that distance strobing must have been ended by that time
...
int atf_WriteMeasInterval(unsigned short iMeasIntervalUS=800); 
int atf_ReadMeasInterval(unsigned short* iMeasInterval);
...
// reports time in seconds from the most recent reset. If Operation lof is not supported
// this call returns error
int atf_ReadCurrentTime ( unsigned int *pTime, unsigned int *pReference ) ;

//Multiplane auto focus
int atf_EnableMpaf(bool bEnable);
int atf_MpafStable(bool bEnable);

// OOA --------------------------------------------------------------------------------------
// serial maintanance functions
bool atf_ooa_Init ();
void atf_ooa_Close ();

// configuration functions. Stage needs to be stationary when functions are called
// OOA is using a default speed/acceleration and user should not change those parameters unless asked by service
bool atf_ooa_SetAccelerationMmSS(float fAccelerationMmSS);
bool atf_ooa_GetAccelerationMmSS(float *fAccelerationMmSS);
bool atf_ooa_SetVelocityMmS(float fVelocityMmS);
bool atf_ooa_GetVelocityMmS(float *fVelocityMmS);

// starts homing. If Wait is true then returns true when hming is done, or false if failed. If
// Wait is false returns true and starts homing state machine. Needs multiple calls to OOA_DoHomeStates()
// to be follwed up with till OOA_DoHomeStates() returns true
bool atf_ooa_DoHome(bool bWait);
// returns true if Homing is completed. Returns false if homing is still continuing. Upon return check
// status to find out if homing is successful.
bool atf_ooa_DoHomeStateMachineAsync(int *pOoaStatus);
// checks if communication with hardware is available
bool atf_ooa_isOK(void);
// obtains status if returns true, or returns false on error
bool atf_ooa_GetStatus(int *pOoaStatus);
// executes relative move of specified number of micrometers. If bWait is true returns when motion is completed
// (true) or when motion failed to performed fully (false). If bWait is false returns true if motion is
// started or false if motion failed to start. If motion is started caller needs to invoke OOA_IsMoveDoneAsync
// on a periodica basis till it returns true.
bool atf_ooa_MoveUm(float fMoveUm, bool bWait);
// executes absolute move to the position specified in micrometers. Can only be called after homing is done. 
// Otherwise will fail and return false. If bWait is true returns when motion is completed
// (true) or when motion failed to performed fully (false). If bWait is false returns true if motion is
// started or false if motion failed to start. If motion is started caller needs to invoke OOA_IsMoveDoneAsync
// on a periodica basis till it returns true.
bool atf_ooa_MoveToPositionUm(float fMoveUm, bool bWait);
// returns true is stage is not moving. In this case check OoaStatus to see if OoaStatusLastMove.... is
// OoaStatusLastMoveCompleted or some other error
bool atf_ooa_IsMoveDoneAsync(int *pOoaStatus);
// reads current position in micrometers. Returns true if value read is valid or false otherwise
bool atf_ooa_GetPositionUm(float *pfPositionUm);
// reads current position in microsteps
bool atf_ooa_GetPositionUStep(int *piPositionUStep);

// OOA --------------------------------------------------------------------------------------
// sets window mode to the windowMode value. 
// WindowNear1Enm - obj objective near mode operates in a single line mode. atf set to work near mode 
// WindowNear7Enm - obj objective near mode operates in 7 segment mode. atf set to work near mode 
// WindowAutoEnm - atf allowed to change near and far mode automatically. near mode 1 or 7 segments are unaffacted. 
// WindowFarEnm - atf set to work in far mode. near mode 1 or 7 segments are unaffacted. 
int atf_WindowModeSet(int obj, WindowEnm windowMode);
// returns current windowMode value. 
int atf_WindowModeGet(int obj, WindowEnm *pWindowMode);
// set processing mode to the current obj and surface. see ProcessEnm for detals
int atf_ProcessSet(int obj, ProcessEnm processEnm);
// get current window processing mode
int atf_ProcessGet(int obj, ProcessEnm *pProcessEnm);
