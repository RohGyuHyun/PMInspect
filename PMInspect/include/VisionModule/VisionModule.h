...
#define INSPECTION_SPACE_DISTRIBUTION	2
...
#define INSPECTION_THREAD_TIME_OUT				20000
...
public:
	...
	BOOL GetInspection() { return m_bInspection; };
	void SetInspection(BOOL isInsp) { m_bInspection = isInsp; };
...
public:
	...
	BOOL GetParallelAllInspStartEnd() { return m_bParallelAllInspStartEnd; };
	BOOL GetAutoInspection() { return m_InspPara.bInspection; };
protected:
	long m_Disp;
	BOOL m_bInit;
	BOOL m_bGrabEnd;
	...
	void BYTEToBit(BYTE byte, BOOL *bit);
	void BitToBYTE(BOOL *bit, BYTE *byte);
	...
	CString m_strRsltFilePath;

	int m_nInspCnt;

	BOOL m_bUnderInspEnd;
	BOOL m_bParallelAllInspStartEnd;
	...
	void RsltSumThreadStart();
	BOOL m_bInspRsltSum;
#endif

	CWinThread *m_pThreadFilterImageSave;
	UINT static FilterImageSaveThread(LPVOID pParam);
	BOOL m_bFilterImageSaveThreadStart;
	BOOL m_bFilterImageSaveThreadEnd;
	void ThreadFilterImageSave();
	char m_strFilterImageSavePath[1024];

	CWinThread *m_pThreadGrabImageSave;
	UINT static GrabImageSaveThread(LPVOID pParam);
	BOOL m_bGrabImageSaveThreadStart;
	BOOL m_bGrabImageSaveThreadEnd;
	void ThreadGrabImageSave();
	char m_strGrabImageSavePath[1024];
	...
	double m_dSumThreadStartTime;
	double m_dSumThreadEndTime;
};