...
void CVisionModule::BYTEToBit(BYTE byte, BOOL *bit)
{
	for (int i = 7; i > -1; i--)
	{
		bit[i] = (byte >> i) & 0x00000001;
	}
}

void CVisionModule::BitToBYTE(BOOL *bit, BYTE *byte)
{
	BYTE data = 0;
...
{
	CString strLog;
	strLog.Format(_T("InitThread Start"));
	//WriteLog(strLog, VISION_LOG);
	BOOL rslt = TRUE;
	...
	if (rslt)
	{
		m_pThreadFilterImageSave = AfxBeginThread(FilterImageSaveThread, this, THREAD_PRIORITY_TIME_CRITICAL);
		m_pThreadFilterImageSave->m_bAutoDelete = TRUE;

		if (m_pThreadFilterImageSave == NULL)
		{
			strLog.Format(_T("Filter Image Save Thread Init Error"));
			//WriteLog(strLog, VISION_LOG);
			rslt = FALSE;
		}
		...
		if (rslt)
		{
			m_pThreadGrabImageSave = AfxBeginThread(GrabImageSaveThread, this, THREAD_PRIORITY_TIME_CRITICAL);
			m_pThreadGrabImageSave->m_bAutoDelete = TRUE;

			if (m_pThreadGrabImageSave == NULL)
			{
				strLog.Format(_T("Grab Image Save Thread Init Error"));
				//WriteLog(strLog, VISION_LOG);
				rslt = FALSE;
			}
		}
	...
	strLog.Format(_T("InitThread End"));
	//WriteLog(strLog, VISION_LOG);
	return rslt;
}
...
{
	CString strLog;
	strLog.Format(_T("InitRslt Start"));
	//WriteLog(strLog, VISION_LOG);
	m_bRsltView = FALSE;
...
{
	CString strLog;
	strLog.Format(_T("InitVision Start"));
	//WriteLog(strLog, VISION_LOG);
	BOOL rslt = TRUE;
	m_bFilterImageView = FALSE;
	...
	{
		m_InspRslt[i].hInsp_rgn.Reset();
		m_InspRslt[i].hShiftInsp_rgn.Reset();
	}
	...
	m_nInspThreadIdx = 0;
	strLog.Format(_T("InitVision End"));
	//WriteLog(strLog, VISION_LOG);
	return rslt;
}
...
{
	CString strLog;
	strLog.Format(_T("CloseVision Start"));
	//WriteLog(strLog, VISION_LOG);
	BOOL rslt = TRUE;
	...
	m_bFilterImageSaveThreadStart = FALSE;
	m_bFilterImageSaveThreadEnd = TRUE;
	m_bGrabImageSaveThreadStart = FALSE;
	m_bGrabImageSaveThreadEnd = TRUE;
	Sleep(100);
	...
	if (m_nMainInspType == INSPECTION_TYPE_PARALLEL)
	{
		...
		for (int i = 0; i < MAX_PARALLEL_THREAD; i++)
		{
			...
			{
				DWORD dwRet = WaitForSingleObject(m_pThreadParallelInsp[i]->m_hThread, INSPECTION_THREAD_TIME_OUT);
	...
	strLog.Format(_T("CloseVision End"));
	//WriteLog(strLog, VISION_LOG);
...
BOOL CVisionModule::SelectInspectionSpace(int nSpaceType)
{
	...
	case INSPECTION_SPACE_PM_COUNT:
		strTemp.Format(_T("PM Cnt Insp Rgn Teaching"));
			break;
	case INSPECTION_SPACE_PM_SHIFT:
		strTemp.Format(_T("PM Shift Insp Rgn Teaching"));
		break;
	case INSPECTION_SPACE_DISTRIBUTION:
		strTemp.Format(_T("PM Distribution Insp Rgn Teaching"));
		break;
	}
	strLog.Format(_T("SelectInspectionSpace Type(%s) Start"), strTemp);
	//WriteLog(strLog, VISION_LOG);
	...
	Hobject hCrop_image, hIlluminate, hThreshold, hRectangle, hEmphasize, hLaplace, hScale, hCrop_image2, hDistritionRgn, hCropImage;
	Hlong lRectCnt;
	...
	int i = m_nInsp_Idx;
	{
		...
		case INSPECTION_SPACE_PM_COUNT_TEMP:
			...
			if (mean_image(hCrop_image, &hEmphasize, insPara.ImageFilterPara[i].nMeanSize, insPara.ImageFilterPara[i].nMeanSize) != H_MSG_TRUE)
			{
				break;
			}

			if (emphasize(hEmphasize, &hCrop_image2, insPara.ImageFilterPara[i].nMeanSize, insPara.ImageFilterPara[i].nMeanSize, 3) != H_MSG_TRUE)
			{
				break;
			}

			if (laplace_of_gauss(hCrop_image2, &hLaplace, insPara.InspRectRecipe[i].dLaplaceSigma) != H_MSG_TRUE)
			{
				break;
			}
			...
			crop_rectangle1(hScale, &hCropImage, (Hlong)frist_rect.top - 50, (Hlong)frist_rect.left, (Hlong)last_rect.bottom + 50, (Hlong)last_rect.right);
			get_image_pointer1(hCropImage, (Hlong*)&bTempFilter2, typ, &width, &height);
			...
			lheight -= 5;
			select_shape(hThreshold, &hThreshold, "area", "and", lspace, "max");
			select_shape(hThreshold, &hThreshold, "width", "and", lspace, "max");
			select_shape(hThreshold, &hThreshold, "height", "and", lspace, "max");
			...
			for (int y = 0; y < lRectCnt; y++)
			{
				...
				select_rect.SetRect(frist_rect.left, (long)(Hlong)row0[0] + 0, frist_rect.right, (long)(Hlong)row1[0]);

				if (y > 0)
				{
					if ((select_rect.top - insPara.InspRectRecipe[i].InspRect[y - 1].lRectBottom) <= lspace)
					{
						select_rect.SetRect(frist_rect.left, insPara.InspRectRecipe[i].InspRect[y - 1].lRectBottom + lspace, frist_rect.right, insPara.InspRectRecipe[i].InspRect[y - 1].lRectBottom + lspace + select_rect.Height());
					}
				}
			...
			for (int y = 1; y < 1999; y++)
			{
				...
				{
					temp_rect2.SetRect(temp_rect.left, temp_rect.bottom + lspace, temp_rect.right, temp_rect.bottom + lspace + frist_rect.Height());
				}
	...
	strLog.Format(_T("SelectInspectionSpace Type(%s) End"), strTemp);
	//WriteLog(strLog, VISION_LOG);
...
void CVisionModule::RsltSumThreadStart()
{
	if (m_InspPara.bFilterImgView)
	{
		if (m_pThreadParallelInspRsltSum != NULL)
		{
			DWORD dwRet = WaitForSingleObject(m_pThreadParallelInspRsltSum->m_hThread, INSPECTION_THREAD_TIME_OUT);

			if (m_pThreadParallelInspRsltSum)
			{
				delete m_pThreadParallelInspRsltSum;
				m_pThreadParallelInspRsltSum = NULL;
			}
		}

		m_pThreadParallelInspRsltSum = AfxBeginThread(ParallelInspRsltSumThread, this, THREAD_PRIORITY_HIGHEST, 0, CREATE_SUSPENDED);
		m_pThreadParallelInspRsltSum->m_bAutoDelete = FALSE;
		m_pThreadParallelInspRsltSum->ResumeThread();
		CString strLog;
		strLog.Format(_T("ParallInspRsltSum Thread Start"));
		//WriteLog(strLog, VISION_LOG);
	}
}
...
{
	CString strLog;
	strLog.Format(_T("SumInspRslt Start"));
	//WriteLog(strLog, VISION_LOG);
	...
	int nIdx;
	BOOL bCntRslt = TRUE;
	BOOL bDistributionRslt = TRUE;
	BOOL bShiftRslt = TRUE;
	BOOL bSLRslt = TRUE;
	BOOL bRsltBit[BYTE_TO_BIT_SIZE];
	int j = 0, l = 0;
	int nRsltCnt = 0;
	BOOL bSearch = FALSE;
	BYTE byRslt = 0;
...
//#pragma omp parallel for private(m_Inspection) reduction(hShiftRslt, hInspRslt, hDispImage)
	...
	for (int i = 0; i < m_nInspThreadIdx; i++)
	{
		...
		for (int j = 0; j < m_Inspection[i]->GetInspRectCnt();j++)
		{
			...
			m_PmEachRslt[nIdx].byRslt = 0;
			memset(bRsltBit, FALSE, sizeof(BOOL) * BYTE_TO_BIT_SIZE);
			for (int k = 0; k < 4; k++)
			{
				if (m_InspPara.ShiftInspRect[k][nIdx].isInsp)
				{
					...
					if (!m_InspRslt[m_nInsp_Idx].bShiftInspRslt[k][nIdx])
					{
						bRsltBit[k + 1] = TRUE;
						bShiftRslt = FALSE;
					}
			...
			BitToBYTE(bRsltBit, &m_PmEachRslt[nIdx].byRslt);
			...
			for (int y = 0; y < m_InspPara.PMDistributionInspPara.nDistributionYCnt; y++)
			{
				for (int x = 0; x < m_InspPara.PMDistributionInspPara.nDistributionXCnt; x++)
				{
					...
					bDistributionRslt = TRUE;
				}
			}
		}
		union2(hShiftRslt, *m_Inspection[i]->GetShiftRsltRgn(), &hShiftRslt);

		PmCntRslt = m_Inspection[i]->GetPMCntRslt();
		m_PmCntRslt.push_back(PmCntRslt);
		for (int j = m_Inspection[i]->GetInspRectIdx(0); j < (m_Inspection[i]->GetInspRectIdx(0) + m_Inspection[i]->GetInspRectCnt()); j++)
		{
			m_PmEachRslt[j].nCnt = PmCntRslt.lRsltCnt[j - m_Inspection[i]->GetInspRectIdx(0)];
			memset(bRsltBit, FALSE, sizeof(BOOL) * BYTE_TO_BIT_SIZE);
			BYTEToBit(m_PmEachRslt[j - m_Inspection[i]->GetInspRectIdx(0)].byRslt, bRsltBit);
			if (!PmCntRslt.bRslt[j - m_Inspection[i]->GetInspRectIdx(0)])
			{
				bCntRslt = FALSE;
				bRsltBit[0] = TRUE;
				BitToBYTE(bRsltBit, &m_PmEachRslt[j].byRslt);
			}
		}
	}
	...
	m_bCntRslt = bCntRslt;
	m_bShiltRslt = bShiftRslt;
	m_bDistributiconRslt = bDistributionRslt;
	m_bSLRslt = bSLRslt;

	m_bPMCntRslt[m_nInsp_Idx] = TRUE;
	if (!m_bCntRslt || !m_bShiltRslt || !m_bDistributiconRslt || !m_bSLRslt)
	{
	...
	if (!m_InspPara.bFilterImgView)
	{
		set_check("~give_error");
		m_bFilterImageView = FALSE;
		if (disp_obj(m_HalGrabImage, m_Disp) != H_MSG_TRUE)
		{
...
//#pragma omp parallel for private(m_RsltImgRect, m_RsltFilterImg) reduction(|:m_HalDispFilterImage)
	...
	strLog.Format(_T("SumInspRslt End"));
	//WriteLog(strLog, VISION_LOG);
}
...
BOOL CVisionModule::SetInspImageSize(int nEndGrabHeight, int nStartYIdx, int nEndYIdx, int nParallelInspIdx)
{
	...
	while (TRUE)
	{
		if (m_InspPara.InspRectRecipe[m_nInsp_Idx].InspRect[nEndYIdx].lRectBottom == 0)
		{
			nEndYIdx--;
		}
		else
...
void CVisionModule::InspInit(int nThreadCnt)
{
	for (int nInsThreadIdx = 0; nInsThreadIdx < nThreadCnt; nInsThreadIdx++)
	{
		...
		{
			m_Inspection[nInsThreadIdx] = new CInspection();
		}
	}
}
...
{
	BOOL bInspEnd = FALSE, bInspInit = FALSE, bImage = TRUE, bImageCopy = FALSE;
	int i = 0, j = 0, k = 0, o = 0;
	CRect rect[1000];
	double s_time, e_time;
	CString strText, strText1, strText2, strLog;
	...
	while (TRUE)
	{
		if (m_bParallelMainInspThreadStart)
		{
			...
			if (m_bInspectionEnd)
			{
				...
				m_bInspectionEnd = TRUE;
				bInspInit = FALSE;
				m_bParallelAllInspStartEnd = FALSE;
			}
			...
			if (g_MilCtrl.GetGrabLineCnt() > 0 && m_bInspectionStart)
			{
				...
				{
					strLog.Format(_T("Main Inspection Thread Start"));
					WriteLog(strLog, VISION_LOG);
					...
					if (!GetTestInsp())
					{
						gen_image_proto(m_HalGrabImage, &m_HalGrabImage, 0);
						gen_image_proto(m_HalDispImage, &m_HalDispImage, 0);
					}
					...
					{
						SelectInspectionSpace(INSPECTION_SPACE_PM_COUNT_TEMP);
						SelectInspectionSpace(INSPECTION_SPACE_PM_SHIFT);
					}
					SelectInspectionSpace(INSPECTION_SPACE_DISTRIBUTION);

					strLog.Format(_T("Paralle Inspection Thread Start"));
					WriteLog(strLog, VISION_LOG);
					
					m_bInspRsltSum = FALSE;
					...
					strText2.Format(_T("%02d:%02d:%02d"), NowTime.GetHour(), NowTime.GetMinute(), NowTime.GetSecond());
					m_strRsltFileName.Format(_T("%02d%02d%02d"), NowTime.GetHour(), NowTime.GetMinute(), NowTime.GetSecond());
					m_strRsltFilePath.Format(_T("%s\\%04d\\%02d\\%02d\\"), VISION_IMAGE_PATH, NowTime.GetYear(), NowTime.GetMonth(), NowTime.GetDay());
					if (m_InspPara.bInspImgSave)
					{
						MakeDirectories(m_strRsltFilePath);
					}

					strText.Format(_T("%s%s_InspGrabImage.bmp"), m_strRsltFilePath, m_strRsltFileName);
					sprintf_s(m_strGrabImageSavePath, strText.GetLength() + 1, "%S", strText);

					strText.Format(_T("%s%s_InspFilterImage.bmp"), m_strRsltFilePath, m_strRsltFileName);
					sprintf_s(m_strFilterImageSavePath, strText.GetLength() + 1, "%S", strText);
				...
				//if (nLineCnt >= o)
				{
					if (SetInspImageSize(j + m_lGrabHeight, k * INSP_LEAD_CNT, k * INSP_LEAD_CNT + INSP_LEAD_CNT - 1, k))
					{
						...
						if (!GetTestInsp())
						{
							...
							g_MilCtrl.SetGrabCopyStart(TRUE);
						...
						strLog.Format(_T("SetParallelInsp Start (%d Idx Thread)"), k);
						WriteLog(strLog, VISION_LOG);
				...
				if ((k - 1) * INSP_LEAD_CNT + INSP_LEAD_CNT - 1 > m_InspPara.InspRectRecipe[m_nInsp_Idx].nYRectCnt - 1)
				{
					//	m_bParallelAllInspStartEnd = TRUE;
					//}

					//if(m_bParallelAllInspStartEnd && m_bGrabEnd && !g_MilCtrl.GetGrabCopy(k - 1) && m_Inspection[k-1]->GetUnderInspection())
					//{
					if (!m_InspPara.bFilterImgView)
					{
						g_MilCtrl.SetGrabLineCnt(0);
						...
						if (m_InspPara.bInspImgSave && bImage)
						{
							SaveImage(&m_HalGrabImage, m_strGrabImageSavePath);
						}
						copy_image(m_HalGrabImage, &m_HalDispFilterImage);
					}
					...
					delete bThreadEnd;
					strLog.Format(_T("Paralle Inspection Thread All End"));
					WriteLog(strLog, VISION_LOG);

					strLog.Format(_T("SumInspRslt Start"), k);
					WriteLog(strLog, VISION_LOG);
					SumInspRslt();
					strLog.Format(_T("SumInspRslt End"), k);
					WriteLog(strLog, VISION_LOG);

					while (!m_bInspRsltSum)
					{
						if (!m_InspPara.bFilterImgView)
						{
							m_bInspRsltSum = TRUE;
					...
					if (m_InspPara.bFilterImgView)
					{
						strText.Format(_T("%.3f sec"), (m_dSumThreadEndTime - m_dSumThreadStartTime) / 1000.0);
						strLog.Format(_T("ParallInspRsltSum Thread End ------- Process Time : %s"), strText);
						//WriteLog(strLog, VISION_LOG);
					...
					e_time = GetCurrentTime();
					strLog.Format(_T("%.3f sec"), (e_time - s_time) / 1000.0);
					DisplayRslt();
					if (!bInspEnd)
					{
						WritePMInfoRslt();
						NowTime = CTime::GetCurrentTime();
						strText1.Format(_T("%s,%s"), strText2, strLog);
						...
						for (int z = 0; z < 4; z++)
						{
							...
							strText2.Format(_T("%s,%s"), strText1, strText);
							strText1.Format(_T("%s"), strText2);
						}
					...
					strLog.Format(_T("Main Inspection Thread End -------- TackTime : %s"), strText);
					WriteLog(strLog, VISION_LOG);
...
BOOL CVisionModule::GetParallelThreadEnd(int nThreadIdx)
{
	...
	{
		DWORD dwRet = WaitForSingleObject(m_pThreadParallelInsp[nThreadIdx]->m_hThread, INSPECTION_THREAD_TIME_OUT);
...
void CVisionModule::SetParallelInsp(Hobject *image, int nIdx)
{
	...
	if (crop_rectangle1(hTempImage, &hInspImage, m_InspImageSize[nIdx].top, m_InspImageSize[nIdx].left, m_InspImageSize[nIdx].bottom, m_InspImageSize[nIdx].right - 1) != H_MSG_OK)
	{
		m_Inspection[nIdx]->SetInspection(FALSE);
		return;
	}

	//strLog.Format(_T("%d Idx Inspection Thread Start"), nIdx);
	//WriteLog(strLog, VISION_LOG);

	m_Inspection[nIdx]->SetInspection(TRUE);
...
void CVisionModule::SetParallelInsp(int nIdx)
{
	...
	{
		DWORD dwRet = WaitForSingleObject(m_pThreadParallelInsp[nIdx]->m_hThread, INSPECTION_THREAD_TIME_OUT);
...
UINT CVisionModule::GrabImageSaveThread(LPVOID pParam)
{
	CVisionModule *pdlg = (CVisionModule *)pParam;

	pdlg->ThreadGrabImageSave();

	return 0;
}

void CVisionModule::ThreadGrabImageSave()
{
	while (TRUE)
	{
		if (m_bGrabImageSaveThreadStart)
		{
			m_bGrabImageSaveThreadStart = FALSE;
			Hobject hImage;
			copy_image(m_HalGrabImage, &hImage);
			SaveImage(&hImage, m_strGrabImageSavePath);
		}

		if (m_bGrabImageSaveThreadEnd)
		{
			break;
		}
		Sleep(1);
...
UINT CVisionModule::FilterImageSaveThread(LPVOID pParam)
{
	CVisionModule *pdlg = (CVisionModule *)pParam;

	pdlg->ThreadFilterImageSave();

	return 0;
}

void CVisionModule::ThreadFilterImageSave()
{
	while (TRUE)
	{
		if (m_bFilterImageSaveThreadStart)
		{
			Hobject hImage;
			copy_image(m_HalDispFilterImage, &hImage);
			SaveImage(&hImage, m_strFilterImageSavePath);
			m_bFilterImageSaveThreadStart = FALSE;
		}

		if (m_bFilterImageSaveThreadEnd)
		{
			break;
		}
		Sleep(1);
...
{
	CString strLog;
	strLog.Format(_T("WritePMInfoRslt Start"));
	//WriteLog(strLog, VISION_LOG);
	CStdioFile pFile;
	CTime NowTime = CTime::GetCurrentTime();
	...
	strPath.Format(_T("%s\\%s\\"), VISION_LOG_PATH, strCurtime);
	...
	strLog.Format(_T("WritePMInfoRslt End"));
	//WriteLog(strLog, VISION_LOG);
}
...
{
	CString strLog;
	strLog.Format(_T("WriteRslt Start"));
	//WriteLog(strLog, VISION_LOG);
	CStdioFile pFile;
	...
	//strCurtime = NowTime.Format(_T("%Y")) + "\\" + NowTime.Format(_T("%m")) + "\\" + NowTime.Format(_T("%d"));// 일까지,
	//strPath.Format(_T("%s\\%s\\"), VISION_IMAGE_PATH, strCurtime);
	//MakeDirectories(strPath);//경로에따라 찾고 없으면 만든다...

	//tStr.Format(_T("%sInspGrabImage_%s.bmp"), strPath, m_strRsltFileName);
	//sprintf_s(m_strGrabImageSavePath, tStr.GetLength() + 1, "%S", tStr);

	//tStr.Format(_T("%sInspFilterImage_%s.bmp"), strPath, m_strRsltFileName);
	//sprintf_s(m_strFilterImageSavePath, tStr.GetLength() + 1, "%S", tStr);

	//if (m_InspPara.bInspImgSave)
	//{
	//	m_bGrabImageSaveThreadStart = TRUE;
	//}


	strLog.Format(_T("WriteRslt End"));
	//WriteLog(strLog, VISION_LOG);
}
...
void CVisionModule::ParallelInspRsltSum()
{
	...
	m_bInspRsltSum = FALSE;
	m_dSumThreadStartTime = GetCurrentTime();
	Hobject hDispImage;
	/*Hobject hTempImage[60];
	CRect *rTempRect;
	int nInspMaxThread = m_nInspThreadIdx;
	copy_image(m_HalGrabImage, &hDispImage);

	rTempRect = new CRect[nInspMaxThread];

	for (int i = 0; i < nInspMaxThread; i++)
	{
		copy_image(*m_Inspection[i]->GetRsltFilterImage(), &hTempImage[i]);
		rTempRect[i] = *m_Inspection[i]->GetRsltImagePos();
	}

	for (int i = 0; i < nInspMaxThread; i++)
	{
		ClipImage(&hTempImage[i], &hDispImage, rTempRect[i]);
	}
	...
	int nThreadIdx = 0;
	CRect rect;
	Hobject hTempClipImage;
	while (TRUE)
	{
		if (m_Inspection[nThreadIdx] == NULL)
		{
			break;
		}

		if (!m_Inspection[nThreadIdx]->GetUnderInspection() && m_Inspection[nThreadIdx]->GetInspectionEnd() && !m_Inspection[nThreadIdx]->GetInspectionStart())
		{
			if (nThreadIdx == 0)
			{
				copy_image(m_HalGrabImage, &hDispImage);
			...
			ClipImage(m_Inspection[nThreadIdx]->GetRsltFilterImage(), &hDispImage, m_Inspection[nThreadIdx]->GetRsltImagePos());
			nThreadIdx++;
		}
		...
		if (nThreadIdx == m_nInspThreadIdx)
		{
			break;
		}

		if (((nThreadIdx - 1) * INSP_LEAD_CNT + INSP_LEAD_CNT - 1 > m_InspPara.InspRectRecipe[m_nInsp_Idx].nYRectCnt) &&
			 (!m_Inspection[nThreadIdx]->GetInspection()))
		{
			rect = m_Inspection[nThreadIdx - 1]->GetRsltImagePos();
			rect.top = 0;
			copy_image(hDispImage, &hTempClipImage);
			copy_image(m_HalGrabImage, &hDispImage);
			ClipImage(&hTempClipImage, &hDispImage, rect);
			break;
		}
		Sleep(1);
	}
	...
	if (m_InspPara.bFilterImgView)
	{
		...
		if (disp_obj(m_HalDispFilterImage, m_Disp) != H_MSG_TRUE)
		{
	...
	if (m_InspPara.bInspImgSave)
	{
		m_bFilterImageSaveThreadStart = TRUE;
	}
	m_dSumThreadEndTime = GetCurrentTime();
	m_bInspRsltSum = TRUE;
}
...
CInspection::~CInspection(void)
{
	if(m_pThreadParallelInsp != NULL)
		DWORD dwRet = WaitForSingleObject(m_pThreadParallelInsp->m_hThread, INSPECTION_THREAD_TIME_OUT);
...
{
	CString strLog;
	strLog.Format(_T("%d Idx Inspection ThreadStart"), m_nParallelInspIdx);
	...
	m_bInspectionStart = FALSE;
	m_bInspectionEnd = TRUE;
	strLog.Format(_T("%d Idx Inspection ThreadEnd"), m_nParallelInspIdx);
	////WriteLog(strLog, VISION_LOG);
}
...
{
	CString strLog;
	strLog.Format(_T("%d Idx PMShiftInspection Start"), m_nParallelInspIdx);
	////WriteLog(strLog, VISION_LOG);
	BOOL rslt = TRUE;
	int nRgnOffsetY = ThreadInspPara.InspImageSize.top;
	int nInspIdx = m_nParallelInspIdx;
	...
	for (int i = 0; i < ThreadInspPara.nYRectCnt; i++)
	{
		for (int j = 0; j < 4; j++)
		{
			if (ThreadInspPara.ShiftInspRect[j][i].isInsp)
			{
				...
				if (laplace_of_gauss(hEmphaImage, &hIlluminate1, ThreadInspPara.ImageFilterPara.dLaplaceSigma) != H_MSG_TRUE)
				{
					continue;
				}
...
{
	CString strLog;
	strLog.Format(_T("%d Idx PMCntInspection Start"), m_nParallelInspIdx);
	////WriteLog(strLog, VISION_LOG);
	BOOL rslt = TRUE;
	int nRgnOffsetY = ThreadInspPara.InspImageSize.top;
	int nInspIdx = m_nParallelInspIdx;
	...
	if (ThreadInspPara.bFilterImgView)
	{
	...
	strLog.Format(_T("%d Idx PMCntInspection End"), m_nParallelInspIdx);
	////WriteLog(strLog, VISION_LOG);
...
{
	CString strLog;
	strLog.Format(_T("%d Idx PMDistributionInspection Start"), m_nParallelInspIdx);
	////WriteLog(strLog, VISION_LOG);
	BOOL rslt = TRUE;
	...
	strLog.Format(_T("%d Idx PMDistributionInspection End"), m_nParallelInspIdx);
	////WriteLog(strLog, VISION_LOG);
