...
BOOL	MILCTRL::Create( HWND MsgRecvhWnd, UINT UserMessage, HWND MainDisplayHwnd, LPCTSTR strDCFName, int GrabbedBufSize, BOOL IsColor, MIL_FLOAT WB_R, MIL_FLOAT WB_G, MIL_FLOAT WB_B, BOOL IsFreePath )
{
	//	이미 생성되어있으면 FALSE 리턴
...
*/
	...
	if( MsysAlloc(M_SYSTEM_SOLIOS, 0, M_DEFAULT, &m_Sys) == M_NULL )						//	System 할당
	{
		Release()		;
		return	FALSE	;
	}
	...
	if( MdispAlloc(m_Sys, M_DEFAULT, _T("M_DEFAULT"), M_WINDOWED, &m_MainDisp) == M_NULL )	//	Main Display 할당
	{
		Release()		;
		return	FALSE	;
	}
	...
	/*
	MILCTRL_IF_FAILED( MdigControl( m_Dig, M_GRAB_EXPOSURE_SOURCE, M_SOFTWARE ) )	//	노출 시간 소프트웨어 방식 설정
	{
		Release()		;
		return	FALSE	;
	}*/

	MdigControl(m_Dig, M_GRAB_TIMEOUT, 100000);//M_INFINITE);
	MdigControl(m_Dig, M_GRAB_TRIGGER, M_ACTIVATE);
	MdigControl(m_Dig, M_GRAB_TRIGGER_SOURCE, M_HARDWARE_PORT6);
	...
	//	그랩 버퍼 할당
	if( MbufAllocColor(m_Sys, 1, Width, 10000, BitsPerPixel+M_UNSIGNED,M_IMAGE+M_GRAB+M_PLANAR, &m_GrabBuf) == M_NULL )
	{
		Release()		;
		return	FALSE	;
	}
	...
	MILCTRL_IF_FAILED( MdigControl(m_Dig, M_GRAB_EXPOSURE_MODE, M_LEVEL_HIGH) )
	{
		Release()		;
		return	FALSE	;
	}
...
BOOL	MILCTRL::GrabSync()
{
	...
	MILCTRL_IF_FAILED( MdigControl(m_Dig, M_GRAB_EXPOSURE + M_TIMER1, M_ACTIVATE) )
	{
		g_MILCTRL_CS.Unlock()	;
		return	FALSE			;
	}
	...
	*/
	...
	/*int i = 0, cnt = 65000 / 1000, x = 0;
	MdigGrab(m_Dig, m_MilGrabBuf[0]);
	MdigGrabWait(m_Dig, M_GRAB_END);*/

	/*if (m_bGrabStop)
	{
		MbufFree(m_MilFullGrabBuf);
		m_bGrab = FALSE;
		m_nGrabError = 1;
		return FALSE;
	}*/
	...
	{
		m_bGrab = FALSE;
		MbufCopyClip(m_MilGrabBuf[0], m_MilFullGrabBuf, 0, (CAMERA_GRAB_HEIGHT));
		m_nGrabError = 2;
	}*/
	...
	//		x = 1 - (i % 2);
	//		//Grab
	//		MdigGrab(m_Dig, m_MilGrabBuf[x]);
	//		MdigGrabWait(m_Dig, M_GRAB_END);  //pkh
	...
	//		{
	//			m_bGrab = FALSE;
	//			MbufCopyClip(m_MilGrabBuf[i % 2], m_MilFullGrabBuf, 0, (CAMERA_GRAB_HEIGHT * i));
	//			m_nGrabError = 2;
	//			break;
	//		}*/

	//		MbufCopyClip(m_MilGrabBuf[i % 2], m_GrabBuf, 0, (1000 * i));
	//	}
	//}
...
BOOL	MILCTRL::GetImage(P_MILCTRL_IMGINFO pImage, BOOL AllocNewBuffer /*FALSE*/)
{
	...
	if (AllocNewBuffer)
	{
		...
		if (m_CamBpp <= 8)
		{
			pImage->m_pData = new	BYTE[m_CamWidth*10000];
		}
		else if (m_CamBpp <= 16)
		{
			pImage->m_pData = new	BYTE[m_CamWidth*10000*2];
		}
	}
	else
	{
	...
	if (m_CamBpp <= 8)
	{
		pImage->m_DataSize = m_CamWidth*10000;
	}
	else if (m_CamBpp <= 16)
	{
		pImage->m_DataSize = m_CamWidth*10000*2;
	}
	...
	for (int i = 0; i < 10000; i++)
	{
		if (m_CamBpp <= 8)
		{
			memcpy(pImage->m_pData + (i*Pitch), pImgBuf + (i*Pitch), m_CamWidth);
		}
		else if (m_CamBpp <= 16)
		{
			memcpy(pImage->m_pData + (i*Pitch*2), pImgBuf + (i*Pitch*2), m_CamWidth);
		}
	}
