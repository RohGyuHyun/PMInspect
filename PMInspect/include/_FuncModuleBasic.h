...
//Make directory
//extern "C" __declspec(dllexport) void MakeDirectories(CString path);	//	BOOL 반환함수 재정의 해서 사용함
...
extern "C" __declspec(dllexport) void WordToBit(WORD word, BOOL *bit);
extern "C" __declspec(dllexport) void BitToWord(BOOL bit[16], WORD *word);
extern "C" __declspec(dllexport) void WordToString(WORD word, char *str);
extern "C" __declspec(dllexport) void StringToWord(char str[2], WORD *word);


//Glass : 오른쪽 아래가 (0, 0)이다(X, Y 수학 좌표계)
extern "C" __declspec(dllexport) void GetCoordGlassToDisp(double panel_size_x, double panel_size_y, double start_offset_x, double start_offset_y, double pixel_ratio_x, double pixel_ratio_y, double gx, double gy, double *dx, double *dy);

extern "C" __declspec(dllexport) void GetCoordDispToGlass(double panel_size_x, double panel_size_y, double start_offset_x, double start_offset_y, double pixel_ratio_x, double pixel_ratio_y, double dx, double dy, double *gx, double *gy);

extern "C" __declspec(dllexport) void CoordGlassToRobotAll( double offset_x, double offset_y,
															double g_am0_x, double g_am0_y, double g_am1_x, double g_am1_y,
															double *r_am0_x, double *r_am0_y, double *r_am1_x, double *r_am1_y,
															int max_cnt,
															double *g_c_am_x, double *g_c_am_y, double *g_f_am_x, double *g_f_am_y, 
															double *g_f_rect_sx, double *g_f_rect_sy, double *g_f_rect_ex, double *g_f_rect_ey,
															double *r_c_am_x, double *r_c_am_y, double *r_f_am_x, double *r_f_am_y,
															double *r_f_rect_sx, double *r_f_rect_sy, double *r_f_rect_ex, double *r_f_rect_ey);

extern "C" __declspec(dllexport) void GetCoordGlassToRobot(double offset_x, double offset_y, double gx, double gy, double *rx, double *ry);

extern "C" __declspec(dllexport) void CoordRobotToDisp(double panel_size_x, double panel_size_y, int start_offset_x, int start_offset_y, double offset_x, double offset_y, double pixel_ratio_x, double pixel_ratio_y, double robo_x, double robo_y, double *rx, double *ry);
/////////////////////////////////////////////////////////////////////////////
//{{AFX_INSERT_LOCATION}}
